...
// Negate means (0.5 - value) for half-bias
...
// Expand normal means (value * 2 - 1) (i.e 0 - 1 mapped to -1 to 1)
...
static DWORD get_op(GLenum operand, GLchar source, GLboolean is_alpha_combiner)
{
    DWORD flag_invert = 0;
    DWORD flag_alpha = 0;
...
void combiner_set_texture_env(void)
{
    ...
    DWORD shader_program[4] = {
        ...
        NV097_SET_SHADER_STAGE_PROGRAM_STAGE3_CLIP_PLANE, // Always enable clip plane at stage 3
    };
    ...
    for (GLuint i = 0; i < GLI_MAX_TEXTURE_UNITS; i++) {
        ...
        if (!texture_unit->texture_2d_enabled || texture_unit->bound_texture_object == &texture_unit->unbound_texture_object) {
            ...
            // FIXME, double-check this is correct for no action
            shader_program[i] = NV097_SET_SHADER_STAGE_PROGRAM_STAGE0_PASS_THROUGH;
        } else {
            switch (texture_unit->tex_env_mode) {
                ...
                case GL_COMBINE: {
                    ...
                    for (GLuint j = 0; j < 2; j++) {
                        GLboolean is_alpha = (j == 1);

                        DWORD arg0 = get_source(sources[j][0], STAGE_INPUT, i);
                        DWORD arg1 = get_source(sources[j][1], STAGE_INPUT, i);
                        DWORD arg2 = get_source(sources[j][2], STAGE_INPUT, i);

                        GLenum FUNCTION = functions[j];
                        ...
                        switch (FUNCTION) {
                            ...
                            case GL_INTERPOLATE: {
                                ...
                                GLenum OP2 = OP[2];
                                GLenum _OP2 = invert_operand(OP2);
                                *INPUT |= PB_MASK(SRC_A, arg0) | get_op(OP[0], 'A', is_alpha);
                            ...
                            case GL_SUBTRACT:
                                ...
                                *INPUT |= PB_MASK(SRC_B, REG_ZERO) | SRC_B_FLAG_INVERT;
                                *INPUT |=
                                    PB_MASK(SRC_C, arg1) | SRC_C_FLAG_HALF_BIAS_NEGATE | get_op(OP[1], 'C', is_alpha);
                                *INPUT |= PB_MASK(SRC_D, REG_ZERO) | SRC_D_FLAG_INVERT;
                                *OUTPUT |= PB_MASK(OP_AxB_PLUS_CxD, REG_SPARE0);
                                break;
                            ...
                            case GL_DOT3_RGBA:
                                ...
                                if (!is_alpha) {
                                    ...
                                    DWORD OP0 = (OP[0] == GL_ONE_MINUS_SRC_COLOR) ? SRC_A_FLAG_EXPAND_NEGATE
                                                                                  : SRC_A_FLAG_EXPAND;
                                    DWORD OP1 = (OP[1] == GL_ONE_MINUS_SRC_COLOR) ? SRC_B_FLAG_EXPAND_NEGATE
                                                                                  : SRC_B_FLAG_EXPAND;
